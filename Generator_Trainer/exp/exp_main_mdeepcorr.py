...
from target_model import Deepcorr700 
from target_model import Deepcorr100 
...
#     def __init__(self,beta=1, alpha = 1, gamma=1): 
...
class Loss_targeted(nn.Module):
    def __init__(self,beta=1, alpha = 0.75, gamma=0.9): 
        super(Loss_targeted, self).__init__()
        self.beta = beta 
        self.alpha = alpha
        self.gamma = gamma
...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        state_dict = torch.load(f"{self.args.target_model_path}tor700_199_epoch11_acc0.88.pth", map_location=self.device)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                # delta[:, :, [0, 1]], delta[:, :, [2, 3]] = constraint_time, 0*constraint_size
                ...
                #     if(avg_time_ratio < 0.16 and avg_size_ratio < 0.17 and acc_adv < 0.1):
                #         torch.save({
                #                     'epoch': epoch + 1,
            ...
            if((epoch < 4 and epoch>0) or size_ratio < 0.5):
                torch.save({
                            'epoch': epoch + 1,
                            'Time L2 rate':avg_time_ratio,
                            'Size L2 rate':avg_size_ratio,
    ...
    def test(self):
        ...
        checkpoint = torch.load('checkpoints/deepcorr700/mdeepcorr_PatchTST_Deepcorr300_sl96_pl48_dm512_nh8/generator_checkpoint_3_acc_0.914_Gacc0.6250_advipd0.000_advsize1.299.pth', map_location=self.device)
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                oringinal_samples.append(batch_x_original_gpu)
        ...
        original_samples = torch.cat(oringinal_samples, dim=0)

        # 转换为numpy数组
        result_fpath = pathlib.Path(f'target_model/mdeepcorr/Gbase/Gmdeepcorr_advsamples_time{avg_time_ratio:.4f}_size{avg_size_ratio:.4f}.p')
