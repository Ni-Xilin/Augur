...
for sample in dataset:
    # 对每个样本中的8个时间序列数据进行处理
    sample['here'][0]['<-'] = pad_or_truncate(sample['here'][0]['<-'], flow_size)
    sample['here'][0]['->'] = pad_or_truncate(sample['here'][0]['->'], flow_size)
    sample['here'][1]['<-'] = pad_or_truncate(sample['here'][1]['<-'], flow_size)
    sample['here'][1]['->'] = pad_or_truncate(sample['here'][1]['->'], flow_size)
    
    sample['there'][0]['<-'] = pad_or_truncate(sample['there'][0]['<-'], flow_size)
    sample['there'][0]['->'] = pad_or_truncate(sample['there'][0]['->'], flow_size)
    sample['there'][1]['<-'] = pad_or_truncate(sample['there'][1]['<-'], flow_size)
    sample['there'][1]['->'] = pad_or_truncate(sample['there'][1]['->'], flow_size)    
...
model = Net().to(device)
LOADED=torch.load('target_model/deepcorr/deepcorr500/tor500_199_epoch17_acc0.85.pth',map_location=device)
...
result_fpath = pathlib.Path("target_model/deepcorr/deepcorr500/test_index500_result.p")
if False:
